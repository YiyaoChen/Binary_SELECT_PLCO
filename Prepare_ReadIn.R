...
master_plco$psaflag <- ifelse(master_plco$latestPSA < 4, 1, 
                              ifelse(master_plco$latestPSA >= 4 & master_plco$latestPSA <= 10, 2, 3))
...
dt_plco <- subset(master_plco, select = c("plco_id", "latestAge", "latestPSA", "latestDRE", "biopsyflag", "raceflag", 
                                          "fhflag","PriorBioflag","psaflag", "PSA4flag", "confirmed_pros"))
colnames(dt_plco) = c('ID','Age','PSA','DRE','biopsyflag','Race','FH','PB','psaflag','PSA4flag', 'PCa')
dt_select <- subset(master_select, select = c('ID','Age','PSA','DRE','biopsyflag','Race','FH','PB','psaflag',"PSA4flag", 'PCa'))
...
rg_data_select_c <- rg_data_select[complete.cases(rg_data_select[, -c(11)]),] # 33257
...
# Remove non-AA with age less than 55 years old from SELECT
# 33257
# has been remove from SELECT
# rg_data_select_c <- rg_data_select_c[-which(rg_data_select_c$Race == 0 & rg_data_select_c$Age < 55), ] 
# 30245
